...
func getRange(expr string, r bounds) (uint64, error) {
	...
	if start < r.min {
		log.Printf("Beginning of range (%d) below minimum (%d): %s\n", start, r.min, expr)
		return 0, fmt.Errorf("Beginning of range (%d) below minimum (%d): %s ", start, r.min, expr)
	}
	if end > r.max {
		log.Printf("End of range (%d) above maximum (%d): %s\n", end, r.max, expr)
		return 0, fmt.Errorf("End of range (%d) above maximum (%d): %s ", end, r.max, expr)
	}
	if start > end {
		log.Printf("Beginning of range (%d) beyond end of range (%d): %s\n", start, end, expr)
...
func mustParseInt(expr string) (uint, error) {
	...
	if err != nil {
		log.Printf("Failed to parse int from %s: %s\n", expr, err)
		return 0, fmt.Errorf("Failed to parse int from %s: %s ", expr, err)
	}
	if num < 0 {
		log.Printf("Negative number (%d) not allowed: %s\n", num, expr)
